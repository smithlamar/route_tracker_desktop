...
public class MyRoutesModel {
     ...
     * (roughly).
     */
    public static final File MY_ROUTES_PATH = new File("resources.my_routes.json");

    /**
     * Proper type name to pass to Gson when building or grabbing a Json file.
     */
    public static final Type MY_ROUTES_TYPE = new TypeToken<Route[]>() {
    }.getType();

    // Properties
    private final DefaultListModel<Route> myRoutes;
    ...
    public void saveRoutes() throws IOException {
        Files.deleteIfExists(MY_ROUTES_PATH.toPath());
        BufferedWriter writer = new BufferedWriter(new FileWriter(MY_ROUTES_PATH));
        writer.append(new Gson().toJson(myRoutes.toArray(), MY_ROUTES_TYPE));
        writer.flush();
    }
